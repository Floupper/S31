...
Instruction PI = {1, 0, 0, 0,0};

// Lecture du code de l'instruction
if ( (strcmp(argv[1], "1") != 0) && (strcmp(argv[1], "add") != 0)) { printf(" !! Gasp !! Dans assService) Opération non reconnue : %s \n", argv[1]);     return 0; }
...
// Affichage de l'instruction :
  printf("Dans addService) A executer : | %d | %d | %d | %d | %d | %d |\n", PI[0],PI[1],PI[2],PI[3],PI[4],PI[5]);
